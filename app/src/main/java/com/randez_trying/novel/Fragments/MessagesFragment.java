...
import com.randez_trying.novel.Activities.Mains.Match.ViewMatchesActivity;
                    ...
                    response -> {
                        try {
                            ...
                            for (int i = 0; i < o.length(); i++) {
                                ...
                                if (message.getIsMedia().equals("true")) {
                                    if (message.getSender().equals(StaticHelper.me.getPersonalId()))
                                        holder.lastMsg.setText("Я: Медиа");
                                    else holder.lastMsg.setText("Медиа");
                                }
                                else {
                                    if (message.getSender().equals(StaticHelper.me.getPersonalId()))
                                        holder.lastMsg.setText("Я: " + Encrypt.decode(message.getText().getBytes(), dialogId));
                                    else holder.lastMsg.setText(Encrypt.decode(message.getText().getBytes(), dialogId));
                                }
