...
import com.randez_trying.novel.Activities.Mains.Adapters.CardStackAdapter;
import com.randez_trying.novel.Activities.Mains.Settings.FiltersSettingsActivity;
...
public class SearchFragment extends Fragment {
    ...
    private void checkLike() {
        Runnable runnable = this::checkLike;

        if (StaticHelper.like) {
            User user = users.get(manager.getTopPosition() - 1);
            if (!iAlreadyLiked(user)) sendLike(user);
            if (checkMatch(user)) {
                Intent intent = new Intent(requireActivity().getApplicationContext(), MatchActivity.class);
            ...
            StaticHelper.like = false;
        }

        likeHandler.postDelayed(runnable, 1);
    }
