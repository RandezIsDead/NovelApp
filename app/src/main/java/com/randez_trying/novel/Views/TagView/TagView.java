...
import static com.randez_trying.novel.Views.TagView.Utils.dp2px;
...
public class TagView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mTextDirection == View.TEXT_DIRECTION_RTL) {
            if (mTagSupportLettersRTL){
                float tmpX = (float) (isEnableCross() ? getWidth() + getHeight() : getWidth()) / 2
                        + fontW / 2;
                for (char c : mAbstractText.toCharArray()) {
                    String sc = String.valueOf(c);
                    tmpX -= mPaint.measureText(sc);
                    canvas.drawText(sc, tmpX, (float) getHeight() / 2 + fontH / 2 - bdDistance, mPaint);
                }
            }else {
                canvas.drawText(mAbstractText,
                        (isEnableCross() ? getWidth() + fontW : getWidth()) / 2 - fontW / 2,
                        (float) getHeight() / 2 + fontH / 2 - bdDistance, mPaint);
            }
        } else {
            canvas.drawText(mAbstractText,
                    (float) (isEnableCross() ? getWidth() - getHeight() : getWidth()) / 2 - fontW / 2 + (isEnableImage() ? (float) getHeight() / 2 : 0),
                    (float) getHeight() / 2 + fontH / 2 - bdDistance, mPaint);
        }
        drawCross(canvas);
        drawImage(canvas);
    ...
    private void drawCross(Canvas canvas){
        if (isEnableCross()){
            mCrossAreaPadding = Math.min(mCrossAreaPadding, (float) getHeight() / 2);
